...
class ReleaseToolTests(unittest.TestCase):
    def test_version_declarations_agree(self) -> None:
        self.assertEqual(project_version(Path.cwd()), "0.1.2")
        self.assertEqual(validate_release_contract(Path.cwd(), "v0.1.2"), "0.1.2")
    ...
    def test_version_mismatch_fails(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            (root / "src/omfg/__init__.py").write_text('__version__ = "0.1.2"\n', encoding="utf-8")
    ...
    def test_build_is_reproducible_and_independently_validated(self) -> None:
        ...
        with (
            ...
            first, first_digest = build(root, first_dir, "v0.1.2", allow_dirty=True)
            second, second_digest = build(root, second_dir, "v0.1.2", allow_dirty=True)
            self.assertEqual(first_digest, second_digest)
            for name in ("omfg-0.1.2.tar.gz", "omfg-0.1.2.tar.gz.sha256", "SHA256SUMS", "install"):
            ...
            self.assertFalse(any(name.startswith("omfg-0.1.2/tests/") for name in names))
            self.assertFalse(any(name.startswith("omfg-0.1.2/.github/") for name in names))

    def test_sha256sums_covers_archive_and_installer_only(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            output = Path(raw)
            archive, archive_digest = build(Path.cwd(), output, "v0.1.2", allow_dirty=True)
    ...
    def test_installer_rendering_is_deterministic_and_recomputes_archive_digest(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            archive = root / "omfg-0.1.2.tar.gz"
            ...
            first = root / "first"
            second = root / "second"
            first_digest = build_installer(template, "0.1.2", archive, first)
            second_digest = build_installer(template, "0.1.2", archive, second)
    ...
    def test_installer_rejects_invalid_digest_and_duplicate_declarations(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "64 lowercase"):
            render_installer(template, "0.1.2", "A" * 64)
        rendered = render_installer(template, "0.1.2", "a" * 64)
    ...
    def test_installer_rejects_wrong_archive_filename(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            with self.assertRaisesRegex(ValueError, "must be named"):
                build_installer(template, "0.1.2", archive, root / "install")

    def test_published_installer_has_literal_digest_and_no_checksum_fallback(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            output = Path(raw)
            archive, digest = build(Path.cwd(), output, "v0.1.2", allow_dirty=True)
            installer = (output / "install").read_text(encoding="utf-8")
            self.assertEqual(installer.count('readonly OMFG_VERSION="0.1.2"'), 1)
    ...
    def test_site_uses_release_installer_and_contains_only_distribution_surface(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as assets_raw, tempfile.TemporaryDirectory() as site_raw:
            ...
            build(root, assets, "v0.1.2", allow_dirty=True)
            site = Path(site_raw) / "site"
            build_site(root, assets, site, "v0.1.2", skip_runtime_validation=True)
            files = {
            ...
            self.assertEqual(
                files,
                {
                    "index.html",
                    "install",
                    "releases/v0.1.2/install",
                    "releases/v0.1.2/SHA256SUMS",
                    "releases/v0.1.2/omfg-0.1.2.tar.gz",
                    "releases/v0.1.2/omfg-0.1.2.tar.gz.sha256",
                },
            )
    ...
    def test_site_rejects_installer_hash_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as assets_raw, tempfile.TemporaryDirectory() as site_raw:
            assets = Path(assets_raw)
            build(Path.cwd(), assets, "v0.1.2", allow_dirty=True)
    ...
    def test_release_notes_preflight_rejects_missing_empty_and_wrong_version(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            with self.assertRaisesRegex(ValueError, "missing"):
                validate_release_notes(root, "0.1.2")
    ...
    def test_validator_rejects_links(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as raw:
            directory = Path(raw)
            archive = directory / "omfg-0.1.2.tar.gz"
            payload = io.BytesIO()
            epoch = int(
                subprocess.run(
            ...
            with tarfile.open(fileobj=payload, mode="w", format=tarfile.USTAR_FORMAT) as bundle:
                link = tarfile.TarInfo("omfg-0.1.2/link")
            ...
            build_installer(template, "0.1.2", archive, directory / "install")
